...
# Azure Configuration for PDF parsing (optional)
AZURE_OPENAI_ENDPOINT = Variable.get(
    "AZURE_OPENAI_ENDPOINT", default_var=os.getenv("AZURE_OPENAI_ENDPOINT")
)
AZURE_OPENAI_API_KEY = Variable.get(
    "AZURE_OPENAI_API_KEY", default_var=os.getenv("AZURE_OPENAI_API_KEY")
)
AZURE_API_VERSION = Variable.get(
    "AZURE_API_VERSION", default_var=os.getenv("AZURE_API_VERSION")
)
AZURE_API_KEY = Variable.get("AZURE_API_KEY", default_var=os.getenv("AZURE_API_KEY"))

# Anthropic Configuration (for Bedrock)
ANTHROPIC_API_KEY = Variable.get(
    "ANTHROPIC_API_KEY", default_var=os.getenv("ANTHROPIC_API_KEY")
)

# Processing Configuration
USE_SAAS_PDF_PARSER = Variable.get(
    "USE_SAAS_PDF_PARSER", default_var=False, deserialize_json=True
)
MODELS_TO_CLEAN = Variable.get(
    "MODELS_TO_CLEAN", default_var=["gpt-4.1"], deserialize_json=True
)
MAX_RETRIES = 3
...
def pdf_to_markdown_parser():
    ...
    def load_metadata() -> list[dict[str, Any]]:
        ...
        """
        setup_python_path()
        from src.documents.metadata import load_metadata as load_metadata_func

        log_event("metadata.load.start")

        try:
            # Load metadata DataFrame
            metadata_df = load_metadata_func()
